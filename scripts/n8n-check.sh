...
# Load API key from n8nac credential store (works on both Windows and WSL)
CRED_FILE="$HOME/.config/n8nac-nodejs/Config/credentials.json"
if [ ! -f "$CRED_FILE" ]; then
    CRED_FILE="$HOME/AppData/Roaming/n8nac-nodejs/Config/credentials.json"
fi
N8N_HOST="${N8N_HOST:-http://172.31.224.1:5678}"
API_KEY=$(python3 -c "import json; c=json.load(open('$CRED_FILE')); hosts=c.get('hosts',{}); print(hosts.get('$N8N_HOST', hosts.get('http://localhost:5678', '')))")
BASE="$N8N_HOST/api/v1"
WF_ID="${1:-}"
LIMIT="${2:-3}"
